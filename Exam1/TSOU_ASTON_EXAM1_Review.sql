--P1
SELECT employee_id, last_name AS "Last Name", salary AS "Current Salary", round(salary+ salary*0.15,0) AS "New Salary", round(salary*0.15,0) AS "Increase"
FROM employees
WHERE round(salary+ salary*0.15,0) BETWEEN &minsalary AND &maxsalary
ORDER BY round(salary+ salary*0.15,0) DESC;

--P2
SELECT RPAD(s_last || ', ' || s_first, 22, '*') || ROUND(((SYSDATE - s_dob)/365),0) AS "Student Name and Age",
       s_pin AS "Pin", REVERSE(TO_CHAR(s_pin)) AS "Coded Pin"
FROM student 
ORDER BY s_last;

--P3
SELECT s_id AS "Student Number", c_sec_id AS "Section", NVL(grade, '--') AS "Grade"
FROM enrollment
...
SELECT last_name || ', ' || first_name AS "Employee", 
       ...
       NEXT_DAY(ADD_MONTHS(hire_date, 6), 'WEDNESDAY') AS "6 Months Review"
FROM employees
